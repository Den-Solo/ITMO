//MergeSort Соловьев Денис (Solovyov Denis), N3251
...
inline Node* MergeSort(Node* data, unsigned size) {
    ...
    while (len < size) {
        //инварианта: src разбит на пары подмассивов
        unsigned i = 0;
        while (i < size - len) {
            unsigned len2 = len;
            if (i + len + len2 > size) {
...
public:
    ...
    ~MappedFile() {
        if (hMapObj != INVALID_HANDLE_VALUE && hMapObj != NULL) {
            UnmapViewOfFile(hMapObj);
            CloseHandle(hMapObj);
        }
    ...
    char* makeMappedFile(const char* name, bool if_write) {

        hFile = CreateFileA(
            name,
            if_write ? GENERIC_WRITE : GENERIC_READ,
            if_write ? FILE_SHARE_WRITE : FILE_SHARE_READ,
            NULL,
            if_write ? CREATE_ALWAYS : OPEN_EXISTING,
            FILE_ATTRIBUTE_NORMAL | (if_write ? FILE_FLAG_SEQUENTIAL_SCAN : FILE_FLAG_RANDOM_ACCESS),
            NULL
        );

        if (hFile == INVALID_HANDLE_VALUE) {
            #ifdef ERROR_LOGGER
                cerr << ("CreateFileA("s + name + "INVALID_HANDLE_VALUE: Input file not found\n");
            #endif
                throw std::runtime_error("CreateFileA("s + name + "INVALID_HANDLE_VALUE: Input file not found\n");//fail file
        }
        ...
        if (!(GetFileSizeEx(hFile, &file_size) && file_size.QuadPart != 0)) {
            #ifdef ERROR_LOGGER
                  cerr << ("GetFileSizeEx error or empty: "s + name + "\n");
            #endif
            throw std::runtime_error("GetFileSizeEx error or empty: "s + name + "\n");//fail file
        }

        hMapObj = CreateFileMappingA(
            hFile,
            NULL,
            if_write ? PAGE_READWRITE : PAGE_READONLY,
            0,
            0,
            if_write ? "lab_1_output" : "lab_1_input"
        );
        
        if (hMapObj == NULL) {
            #ifdef ERROR_LOGGER
                        cerr << ("hMapObj error: "s + name + "\n");
            #endif
            throw std::runtime_error("hMapObj error: "s + name + "\n");//fail
        }

        data = (char*)MapViewOfFile(hMapObj, if_write ? FILE_MAP_WRITE : FILE_MAP_READ, 0, 0, 0);
        return data;
    }
...
int main(int argc, char* args[]) {
    ...
    char* input_ptr;
    try {
        input_ptr = inp.makeMappedFile(args[1], false);
    }
    ...
    {//Writing------------------------------------
        ...
        char* out_ptr;
        try {
            out_ptr = out.makeMappedFile(args[2], true);
        }
        catch (std::runtime_error& e) {
            return 2;
        }
