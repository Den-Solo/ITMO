...
namespace HW8 {
	...
	double x0 = 0;	//начало диапазона интерполируеиой последовательности

	vector<vector<double>> fin_diff_cache;  //конечные разности, таблица; интерполяция ньютона 1 и 2 тип (динамическое программирование)
	...
	public:
		...
		static double Poly(const vector<double>& N_poly, double q, size_t limit = 0) {
			if (!limit)
				limit = N_poly.size();
			else
				limit = min(limit, N_poly.size());
			double res{ N_poly[0] };
			size_t fact{ 1 };
		...
		static vector<double> NewtonInterpolate_1(const vector<double>& data, size_t idx = 0) {
			vector<double> polynom(data.size() - idx);
			for (size_t i = 0; i < polynom.size() - idx; ++i) {
				polynom[i] = fin_diff(data, i, idx);
			}
			return polynom;
		}
	...
	//вторая формула ньютона (для правого конца таблицы)
	class _8_3_2 {

	public:

		//считает интерполированный полином, только для второй формулы ньютона
		static double Poly(const vector<double>& N_poly, double q, size_t limit = 0) {
			if (!limit) limit = N_poly.size();
			else limit = min(limit, N_poly.size());
			double res{ N_poly[0] };
			size_t fact{ 1 };
			for (size_t i = 1; i < N_poly.size(); ++i) {
				res += N_poly[i] * q / fact;
				q *= q + 1;
				fact *= i + 1;
			}
		...
		static double fin_diff(const vector<double>& data, size_t degree, size_t pos) {
			...
			if (!degree) {
				...
				return fin_diff_cache[degree][pos] = fin_diff(data, degree - 1, pos + 1)
					- fin_diff(data, degree - 1, pos);
			}
		}
		...
		static vector<double> NewtonInterpolate_2(const vector<double>& data, size_t idx = 0) {
			vector<double> polynom(data.size() - idx);
			for (size_t i = 0; i < data.size() - idx; ++i) {
				polynom[i] = fin_diff(data, i, polynom.size() - 1 - i);
			}
			return polynom;
		}
		...
		static vector<double> deriv(const vector<double>& data, size_t degree, double h) {
			...
			//const vector<double> poly = _8_3_2::NewtonInterpolate_2(data, h, 0);
			vector<vector<double>> a;
			a.reserve(data.size() - degree - 1);
			for (size_t i = 0, k = data.size() - degree - 2; i < data.size() - degree - 1; ++i) {
				a.emplace_back(_8_3_2::NewtonInterpolate_2(data, k--));
			}
			double xn = x0 + h * data.size();
			double x = x0;
			for (size_t i = 0, j = 0, k = data.size() - degree - 2; i < res.size(); ++i) {
				xn = h * k++;
				res[i] = _deriv(a[j], degree, (0.1) / h, h) / pow(h, degree);
				x += h;
				if (i % 2) ++j;
			}
	...
	namespace Test_8 {
		...
		)
		{
			...
			fin_diff_cache.assign(cnt_intervals + 1, vector<double>(cnt_intervals + 1));
			HW8::x0 = funcs[func_idx]->beg();
			...
			for (size_t i = 0; i < res.size(); ++i) {
				ostr << funcs[func_idx]->beg() + (i + 1) * h2 << ';'
					<< res[i] << ";\n";
			}
		}
		...
		void PrepareCSVs() {
			static const map<string, deriv_f_ptr>
				deriv_funcs = { {"_8_1_1_deriv_", _8_1_1::deriv},
								{"_8_1_2_deriv_", _8_1_2::deriv},
			...
			for (auto& f : deriv_funcs)
				for (size_t func_idx = 0; func_idx < 2; ++func_idx)
...
int main() {
...

	{
		...
		auto f = f1{};
		vector<double> seq = GenSqnc(f, 32);
		cout << _8_1_1::deriv(seq, 1, (f.end() - f.beg()) / 32);
...
void test_7_1_2() {
	using namespace HW8;
	//x0 - правая граница диапазона в общем случае

	{//works fine, x = [1000, 1050]
		x0 = 1050;
		const double h = 10;
		vector<double> seq = { 3.0000000, 3.0043214, 3.0086002, 3.0128372, 3.0170333, 3.0211893 };
		fin_diff_cache.resize(6 + 1, vector<double>(6 + 1));
		auto res = _8_3_2::NewtonInterpolate_2(seq);
		cout << _8_3_2::Poly(res, (1045 - x0) / h);
	}


	{// точность только до сотых 
		x0 = ff{}.end();
		auto seq = Function::GenSqnc(ff{}, 5);
		fin_diff_cache.resize(5 + 1, vector<double>(5 + 1));
		const double h = (ff{}.end() - ff{}.beg()) / (seq.size() - 1);
		auto res = _8_3_2::NewtonInterpolate_2(seq);
		cout << res << "\n\n";
		cout << _8_3_2::Poly(res, (3.55 - x0) / h);
	}


	{//works fine [0,5] (аналитический вид неизвестен)
		vector<double> seq = { 5.2, 8.0, 10.4, 12.4, 14.0, 15.2 };
		fin_diff_cache.resize(6 + 1, vector<double>(6 + 1));
		const double h = 1;
		x0 = 5;
		auto res = _8_3_2::NewtonInterpolate_2(seq);
		cout << res << "\n\n";
		cout << _8_3_2::Poly(res, (2 - x0) / h);
	}


	{//works fine - интеграл вероятностей // работает нормально только если x0 наиближайшее большее число к искомому иксу
		//x = [1.0, 2.0]
		vector<double> seq = { 0.8427, 0.8802, 0.9103, 0.9340,
								0.9523, 0.9661, 0.9763, 0.9838,
								0.9891, 0.9928, 0.9953 };
		...
		const double h = 0.1;
		x0 = 1.5;
		const size_t idx = 0;//номер значения из seq для которого вычисляются конечные разности
							//чем ближе x0 к числу x, тем выше точность
		auto res = _8_3_2::NewtonInterpolate_2(seq, 5);
		cout << res << "\n\n";
		cout << _8_3_2::Poly(res, (1.43 - x0) / h);
	}
}
